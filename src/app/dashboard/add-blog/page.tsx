...
export default function AddBlogPage() {
  ...
  // Load backend categories if available
  useEffect(() => {
  ...
  const handleSubmit = async (isDraft: boolean) => {
    if (!title || !category || !shortDescription || !content) {
      toast.error("Please fill in all required fields (Title, Category, Short Description, Content)");
      return;
    }
    ...
    try {
      ...
      console.error(error);
      toast.error(error.response?.data?.error || "Failed to submit blog");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Link href="/dashboard" className="p-2 bg-white/5 border border-white/10 rounded-xl text-slate-400 hover:text-white transition-all">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Create Blog Post</h1>
          <p className="text-slate-400 text-xs mt-0.5">Draft a new post or submit it for review by the admin team.</p>
        </div>
      </div>
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  id="blog-editor"
                  rows={15}
                  placeholder="Write your blog post in detail (Markdown syntax is supported)..."
              ...
              <div className="relative group rounded-xl border border-dashed border-white/10 bg-white/5 hover:bg-white/[0.07] overflow-hidden flex flex-col items-center justify-center h-44 cursor-pointer transition-all">
                {coverPreview ? (
                  ...
                  <div className="text-center p-4 flex flex-col items-center">
                    ...
                    <span className="text-[10px] text-slate-600 mt-1">Recommended: 1200 x 630 px</span>
                  </div>
                )}
                <input 
                  type="file" 
              ...
              <div className="relative group rounded-xl border border-dashed border-white/10 bg-white/5 hover:bg-white/[0.07] overflow-hidden flex flex-col items-center justify-center h-32 cursor-pointer transition-all">
                {featuredPreview ? (
                  ...
                  <div className="text-center p-3 flex flex-col items-center">
                    ...
                    <span className="text-xs text-slate-400">Select thumbnail/featured file</span>
                  </div>
                )}
                <input 
                  type="file" 
