...
export default function CertificatesPage() {
	...
	const [activeTab, setActiveTab] = useState<"participants" | "history">(
		"participants"
	);
	...
	// Site Settings for Certificate Header & Branding
	const [siteSettings, setSiteSettings] = useState({
		navbarLogoUrl: "",
		watermarkUrl: "",
		certTopLogoUrl: "",
		certShowBismillah: true,
		certBismillahText: "﷽",
		regFormOrgLine1: "Bangladesh Anjumane Talamije Islamia",
		regFormOrgLine2: "Chhatak Uttar Upazila",
		certMainTitle: "Certificate of Participation",
		certSubTitlePrefix: "This is to certify that",
		certBodyText: "successfully registered and participated in the event",
		certSecurityLabel: "Official Security Verification",
	});
			...
			.then((res) => {
				if (res.data.success && res.data.data) {
					...
					setSiteSettings({
						...
						certTopLogoUrl: d.certTopLogoUrl || "",
						certShowBismillah:
							d.certShowBismillah !== undefined ? d.certShowBismillah : true,
						certBismillahText: d.certBismillahText || "﷽",
						regFormOrgLine1:
							d.regFormOrgLine1 || "Bangladesh Anjumane Talamije Islamia",
						regFormOrgLine2: d.regFormOrgLine2 || "Chhatak Uttar Upazila",
						certMainTitle: d.certMainTitle || "Certificate of Participation",
						certSubTitlePrefix:
							d.certSubTitlePrefix || "This is to certify that",
						certBodyText:
							d.certBodyText ||
							"successfully registered and participated in the event",
						certSecurityLabel:
							d.certSecurityLabel || "Official Security Verification",
					});
				}
			})
			.catch(() => {});
		...
		try {
			const res = await axios.get("/api/admin/certificates", {
				params: {
					...
					event:
						eventFilter === "Active" ? activeEvent?.name || "" : eventFilter,
				},
			});
			if (res.data.success) {
	...
	const toggleSelectAll = () => {
		...
		if (allSelected) {
			setSelectedRegIds((prev) =>
				prev.filter((id) => !pageRegIds.includes(id))
			);
		} else {
			setSelectedRegIds((prev) => {
	...
	const handleGenerateCertificates = async () => {
		...
		try {
			...
			toast.error(
				error.response?.data?.error || "Failed to generate certificates"
			);
		} finally {
			setGenerating(false);
		}
	...
	const handlePrintBulk = () => {
		const certsToPrint = participants.filter((p) =>
		...
		try {
			...
			if (res.data.success && res.data.data.length > 0) {
				const allCerts = res.data.data.map((p: any) => ({
					certificateId:
						p.certificate?.certificateId || `CERT-${p.registrationId}`,
					registrationId: p.registrationId,
					fullName: p.fullName,
					eventName:
						p.certificate?.eventName ||
						res.data.activeEvent?.name ||
						activeEvent?.name ||
						"Event",
					eventDate:
						p.certificate?.eventDate ||
						res.data.activeEvent?.date ||
						activeEvent?.date ||
						"",
					generatedDate:
						p.certificate?.generatedDate || new Date().toISOString(),
				}));
			...
			toast.error(
				error.response?.data?.error ||
					"Failed to fetch all certificates for printing."
			);
		}
	};
			...
			{printCerts.length > 0 && (
				<div
					...
					{printCerts.map((cert, index) => (
						<div
							key={`${cert.certificateId || cert.registrationId}-${index}`}
							className="print-certificate-page font-serif-title">
							...
							<svg
								...
								<path
									d="M5,5 L45,5 L45,12 L12,12 L12,45 L5,45 Z"
									fill="#d97706"
								/>
								<circle
									cx="20"
							...
							<svg
								...
								<path
									d="M95,5 L55,5 L55,12 L88,12 L88,45 L95,45 Z"
									fill="#d97706"
								/>
								<circle
									cx="80"
							...
							<svg
								...
								<path
									d="M5,95 L45,95 L45,88 L12,88 L12,55 L5,55 Z"
									fill="#d97706"
								/>
								<circle
									cx="20"
							...
							<svg
								...
								<path
									d="M95,95 L55,95 L55,88 L88,88 L88,55 L95,55 Z"
									fill="#d97706"
								/>
								<circle
									cx="80"
							...
							<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 opacity-[0.055] pointer-events-none z-0 flex items-center justify-center">
								<img
									src={
										siteSettings.watermarkUrl ||
										siteSettings.navbarLogoUrl ||
										"/bangladesh-anjumane-talamije-islamia-seeklogo.png"
									}
							...
							<div className="text-center mt-3 relative z-10">
								{siteSettings.certShowBismillah && (
									<div className="font-serif text-xl text-emerald-700 font-bold mb-2">
										{siteSettings.certBismillahText || "﷽"}
									</div>
								)}
								<img
									src={
										siteSettings.certTopLogoUrl ||
										siteSettings.navbarLogoUrl ||
										"/bangladesh-anjumane-talamije-islamia-seeklogo.png"
									}
								...
								<p className="text-sm text-slate-600 font-serif-title">
								...
								<div className="flex items-center justify-center my-1.5">
									<svg
										...
										<line
											...
											x2="160"
											y2="7"
											stroke="url(#grad-right-p)"
											strokeWidth="1"
										/>
									</svg>
								</div>
								<p className="text-base italic text-slate-600 mb-1.5 font-serif-title">
									{siteSettings.certSubTitlePrefix}
								</p>
							...
							<div className="text-center mb-1 relative z-10">
								<p className="text-[9px] font-sans text-amber-600 font-bold tracking-widest uppercase mb-1">
									{siteSettings.certSecurityLabel}
								</p>
								...
								<p className="font-mono text-xs font-semibold text-slate-600 mt-1">
									Cert ID: {cert.certificateId} | Reg ID: {cert.registrationId}
								</p>
								<p className="font-mono text-[9px] font-bold text-green-700">
									{getSecurityHash(cert.certificateId, cert.registrationId)}
								</p>
							</div>

							{/* Signatures */}
							<div className="flex justify-between items-end px-10 pb-5 relative z-10 mb-12.5">
								<div className="text-center w-48 font-serif-title">
									<div className="h-9 relative flex items-end justify-center pb-0.5">
										{activeEvent?.presidentSignatureUrl ?
											<img
												src={activeEvent.presidentSignatureUrl}
												className="h-8 max-w-40 object-contain mb-1"
											/>
										:	null}
									</div>
									<div className="border-t border-slate-300 w-32 mx-auto my-1"></div>
									<p className="font-bold text-slate-800 text-sm mt-0.5">
										{activeEvent?.presidentName || "President"}
									</p>
									<p className="text-xs text-slate-500">
										{activeEvent?.presidentTitle || "Chhatak Uttar Upazila"}
									</p>
								</div>
								<div className="text-center w-48 font-serif-title">
									<div className="h-9 relative flex items-end justify-center pb-0.5">
										{activeEvent?.secretarySignatureUrl ?
											<img
												src={activeEvent.secretarySignatureUrl}
												className="h-8 max-w-40 object-contain mb-1"
											/>
										:	null}
									</div>
									<div className="border-t border-slate-300 w-32 mx-auto my-1"></div>
									<p className="font-bold text-slate-800 text-sm mt-0.5">
										{activeEvent?.secretaryName || "General Secretary"}
									</p>
									<p className="text-xs text-slate-500">
										{activeEvent?.secretaryTitle || "Chhatak Uttar Upazila"}
									</p>
								</div>
							</div>
						</div>
						...
						<Table>
							...
							<TableBody>
								...
								:	participants.map((item, index) => {
										const isSelected = selectedRegIds.includes(
											item.registrationId
										);
										...
										return (
											<TableRow
												...
												<TableCell className="text-center">
													<button
														onClick={() =>
															toggleSelectParticipant(item.registrationId)
														}
														className="p-1 focus:outline-none">
														{isSelected ?
															<CheckSquare className="w-4 h-4 text-purple-700" />
