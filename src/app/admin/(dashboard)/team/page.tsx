...
export default function AdminTeamPage() {
	...
	const handleToggleActive = async (member: TeamMember) => {
		try {
			...
			setMembers((prev) =>
				prev.map((m) => (m._id === member._id ? res.data.data : m))
			);
		...
		<div className="space-y-6">
			...
			<div>
				...
				<p className="text-slate-400 mt-1 text-sm">
					নেতাদের যোগ করুন, সম্পাদনা করুন ও সাজান — Home page-এ automatically
					দেখাবে
				</p>
			</div>
				...
				<div className="xl:col-span-1">
					<div className="bg-[#0c0c16] border border-white/5 rounded-2xl p-6 space-y-5">
						<div className="flex items-center gap-2 border-b border-white/5 pb-4">
							<div
								className={`w-8 h-8 rounded-lg flex items-center justify-center ${editingMember ? "bg-amber-500/15" : "bg-violet-500/15"}`}>
								{editingMember ?
							...
							<div>
								...
								<p className="text-[11px] text-slate-500">
									{editingMember ?
										`"${editingMember.name}" সম্পাদনা হচ্ছে`
									:	"নাম, পদবী ও ছবি দিন"}
								</p>
							</div>
						</div>
						<form
							...
							<div className="flex gap-2">
								<button
									type="submit"
									disabled={saving}
									className="flex-1 h-10 bg-linear-to-r from-violet-600 to-indigo-600 hover:opacity-90 text-white rounded-xl text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-50 transition-all">
									{saving ?
										<>
						...
						: members.length === 0 ?
							<div className="flex flex-col items-center justify-center py-20 text-center">
								...
								<p className="font-medium text-slate-400 text-sm">
									এখনো কোনো নেতা যোগ করা হয়নি
								</p>
								<p className="text-xs text-slate-600 mt-1">
									বামের form দিয়ে প্রথম সদস্য যোগ করুন
								</p>
							</div>
									...
									.map((member) => (
										<div
											...
											{member.imageUrl ?
												...
												/>
											:	<div className="w-12 h-12 rounded-xl bg-linear-to-br from-violet-600 to-indigo-600 flex items-center justify-center text-white font-bold text-xl shrink-0">
