...
import { Search, Download, Eye, Loader2, ChevronLeft, ChevronRight, MoreHorizontal, Edit2, Trash2 } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function RegistrationsPage() {
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true);
    try {
      const response = await axios.put(`/api/admin/registrations/${editData.registrationId}`, {
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      const ws = utils.json_to_sheet(rows.map((r: any, index: number) => ({
        "S.N.": index + 1,
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      autoTable(doc, {
        startY: 28,
        head: [["S.N.", "ID", "Name", "Mobile", "School/College", "Class", "Group", "District", "Status", "Date"]],
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={exportPDF} disabled={exportLoading === "pdf"}>
            {exportLoading === "pdf" ? <PdfLoader className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />}
            PDF
          </Button>
        </div>
      </div>
              ...
              {loading ? (
                ...
                data.map((item, index) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell className="max-w-[150px] truncate" title={item.schoolName}>
                      {item.schoolName}
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge variant={item.status === 'Verified' ? 'default' : item.status === 'Pending' ? 'secondary' : 'destructive'} 
                             className={item.status === 'Verified' ? 'bg-green-100 text-green-700 hover:bg-green-100' : ''}>
                        {item.status}
                      </Badge>
                    </TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuGroup>
                            ...
                            <DropdownMenuItem onClick={() => navigator.clipboard.writeText(item.registrationId)}>
                              Copy ID
                            </DropdownMenuItem>
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem render={<Link href={`/success/${item.registrationId}`} target="_blank" />}>
                            <Eye className="mr-2 h-4 w-4" />
                            View Ticket
                          </DropdownMenuItem>
                        </DropdownMenuContent>
